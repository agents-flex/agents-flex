...
- **Markdown 风格输出**：保留标题、段落、表格、分页和幻灯片结构，内嵌图片使用 Base64 Data URI。
...
PDF 输出包含 `--- Page N ---` 分页标记。图片统一转换为 PNG Base64 Markdown。纯扫描 PDF 可以提取页面中的图片，但当前不包含 OCR 文字识别。
...
4. 第一个返回非空内容的解析器结束本次处理。
5. 所有解析器均失败或返回空内容时，服务返回 `null`。
6. 请求结束后自动调用 `DocumentSource.cleanup()` 清理临时资源。
...
- Base64 图片会显著增加输出长度；进入大模型前可按业务需求过滤、存储或替换图片内容。
...
4. 对 Base64 图片单独存储时，可以在分块前将 Data URI 替换为对象存储 URL。
