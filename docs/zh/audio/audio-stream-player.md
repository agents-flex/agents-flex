...
Git 地址： https://github.com/agents-flex/audioStreamPlayer （MIT 开源协议）
...
1.  **格式选择**：
    *   **AAC (MP4)**：兼容性最好，推荐首选。
    *   **Opus (WebM)**：压缩率高，延迟极低，适合 Chrome/Edge 环境。
    *   **避免 MP3**：MP3 帧头信息较大，不利于流式切片无缝拼接。

2.  **处理自动播放策略**：
    *   现代浏览器禁止带声音的自动播放。建议由用户点击“开始对话”按钮触发第一次 `play()` 或 `resume()`，后续的 AI 回复即可顺畅自动播放。

3.  **复用实例**：
    *   在多轮对话场景中，建议复用同一个 `AudioStreamPlayer` 实例。每次新对话开始时，确保上一轮已 `close()`，然后直接 `feed` 新数据即可。

4.  **内存监控**：
    *   虽然库内部有环形缓冲区保护，但在极端网络环境下仍可能产生短暂内存峰值。合理设置 `maxBufferMs` 通常足以平衡体验与性能。
