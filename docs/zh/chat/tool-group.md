...
- 客服同时支持订单、物流、退款等领域，但一次问题通常只涉及其中一个领域。
- 管理员与普通用户看到的工具集合不同。
- 不同模型、租户或套餐需要挂载不同能力和系统约束。
- 工具数量中等，并且可以用明确的业务规则直接判断应暴露哪一组。

如果工具有几十到几百个且无法在请求前确定目标，使用 [ToolSearch](./tool-search.md) 更合适；ToolGroup 的 Matcher 是应用规则，不会调用模型做语义检索。
