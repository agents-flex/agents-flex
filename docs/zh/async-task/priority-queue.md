...
当本轮取得多个到期候选任务时，Store 会优先处理数值更大的任务。
...
::: info Redis Store 的候选窗口
Redis 先按 `scheduledSubmitAt` 从 ZSet 读取最多 `batchSize * 8` 个到期候选，再在这批候选中按上述规则
排序。这样避免每轮加载整个积压队列，但意味着窗口之外的高优先级任务要等后续扫描进入候选集。内存
Store 会排序当前全部到期任务；JDBC Store 在 SQL 查询中先排序，再取得同样大小的有界候选集。
:::
...
三个 Store 都在各自取得的候选集内按相同规则选择，并通过锁、CAS 或 Lua 防止同一任务被重复领取。
Redis 和 JDBC 使用有界候选集，多个 Worker 也可能同时获得不同任务，因此真实外部请求的开始顺序还会
受候选窗口、并发竞争、网络和线程调度影响。

因此优先级提供的是候选集内的领取优先，不是全局严格顺序，更不是供应商完成顺序保证。
