...
持续会话需要保留前一次生成的文件时，可以为 Local Runtime 配置稳定的会话目录：
...
try (LocalSkillRuntime runtime = LocalSkillRuntime.builder()
    ...
    .build()) {
    // Skill 会先复制到 <conversationId>/skills，命令和文件都在该会话目录中运行。
}
```

Local 会返回指向会话副本的新 `Skill.basePath`，bootstrap 以及 Skill 在自身目录创建的脚本都不会修改
原始 Skill 目录。文件工具不能读写其他会话目录。Local Shell 仍拥有 Java 进程用户的宿主机权限，因此该配置只用于降低
文件互串风险，不能替代操作系统或容器隔离。
