...
时，可以使用 Redis。生产环境应根据实际负载独立完成压测。
...
当多个应用实例共同处理任务时，它们必须连接同一个 JDBC 数据库或 Redis 集群：
...
Store 可以避免多个执行线程静默覆盖同一份任务进度，但不能替代退款、发货、发送邮件等业务操作自身的
...
这段 SQL 只用于展示版本条件。完整实现还需要原子更新任务状态、下次执行时间和取消标记
...
Store 通过版本号 CAS 防止旧 Snapshot 覆盖新进度。业务层需要确保同一 Turn 不被多个执行线程同时推进；
发生版本冲突时重新加载最新 Snapshot，再决定重试、放弃或修复。
...
旧版 JDBC 表可能还保留 `lease_owner`、`lease_id` 和 `lease_until` 列。新版 Agent Store 不再读取或写入这些
列；初始化 Schema 时会尝试把遗留的 `lease_until` 改为可空，以兼容旧表。新建表不会再创建这些列。升级后
应确认应用账号拥有修改表结构的权限，并在正式环境提前执行一次迁移验证。
...
3. Runner、业务调度器和回调服务必须连接同一套 Store。
