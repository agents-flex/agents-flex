...
`AgentContextCompressionState` 保存摘要和覆盖游标；`AgentContextCompressionTrigger` 决定本次是否达到条件；`AgentContextCompressionCoordinator` 负责组合旧摘要与新增消息、调用压缩器并通过 CAS 保存新状态。
