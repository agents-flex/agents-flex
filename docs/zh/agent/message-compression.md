...
当压缩条件不是固定消息数量，还可以使用 `AgentContextCompressionDecider` 和业务侧 Store，按 Token、Turn、工具结果大小、时间或租户配额做增量压缩。
...
`AgentContextCompressionState` 保存摘要和覆盖游标；`AgentContextCompressionDecider` 决定本次是否达到条件；策略内部的处理器负责组合旧摘要与新增消息、调用压缩器并通过 CAS 保存新状态。
...
```java
AgentContextCompressionDecider decider = AgentContextCompressionDeciders.anyOf(
    AgentContextCompressionDeciders.pendingTokensAtLeast(80_000),
    AgentContextCompressionDeciders.pendingTurnsAtLeast(20),
    AgentContextCompressionDeciders.pendingMessagesAtLeast(100));
```
