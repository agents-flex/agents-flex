...
刚开始接入时，通常从 `run(...)` 开始。需要异步执行时，使用 `start(...)` 创建 Turn，再由业务自己的线程池或消息消费者显式推进。
...
| 由业务调度器恢复任务 | `runner.submitResume(...)` | 把任务变回可执行状态，交给业务代码处理 |

`start(...)` 的含义只是“创建并保存任务”。要让它在后台运行，需要由业务代码负责调度并调用 Runner。
