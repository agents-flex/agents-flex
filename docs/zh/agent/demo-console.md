...
description: 使用真实模型构建持续对话、表单输入、工具调用、人工审批和实时事件控制台。
---
...
控制台 Demo 使用真实 OpenAI-compatible ChatModel，展示一个完整交互应用：普通持续对话、只读工具自动执行、工具动态请求表单、有副作用工具人工审批、阻塞 Turn 恢复、每轮独立 Turn、
...
创建工具展示完整 ToolCall 参数并等待明确批准或拒绝。
无法识别的审批输入不会自动当作拒绝。
