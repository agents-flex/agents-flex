...
flowchart TD
    ...
    Validate -->|"通过"| PersistPlan["创建 AgentTaskPlan<br/>保存父 Turn Snapshot"]

    PersistPlan --> Select{"选择下一个 PENDING 任务"}
    Select -->|"存在"| LoadChild["AgentLoader.loadActive<br/>加载目标 Agent"]
    LoadChild --> CreateChild["创建子 AgentTurn<br/>绑定 parentTurnId / rootTurnId / taskId"]
    CreateChild --> AtomicSave["原子保存父 WAITING_FOR_CHILD<br/>与子 READY Snapshot"]
    AtomicSave --> Execution{"执行环境"}

    Execution -->|"同步 runner.run"| SyncChild["当前线程递归推进子 Turn"]
    Execution -->|"AgentWorker"| AsyncChild["父 Turn 返回阻塞<br/>子 Turn 由 Worker 独立领取"]
    AsyncChild --> ChildTurn["子 AgentRunner 完整执行循环"]
    SyncChild --> ChildTurn

    ChildTurn --> ChildState{"子 Turn 结果"}
    ChildState -->|"仍在审批 / 重试等"| ChildBlocked["父 Turn 保持 WAITING_FOR_CHILD"]
    ChildBlocked --> ChildTurn
    ...
    RootSummary --> CompletePlan["计划 COMPLETED<br/>根 Turn COMPLETED"]
...
5. 子 Turn 终止后，`resumeParentFromChild` 幂等地更新任务状态、累计子任务用量，并把受长度限制的结果写回父 Prompt。
...
3. 创建子 Turn，父 Turn 进入 `WAITING_FOR_CHILD`。
4. 子 Turn 完成后，结果写回任务和父 Prompt。
5. 子 Turn 失败时，按策略停止或进入 `REPLANNING`。
