...
| bot ID、conversation ID、account ID、turn ID | Span Attribute | 每次请求变化，基数很高 |
...
Agents-Flex 对常用关联 ID 使用统一名称：

| Java 常量 | Span Attribute | 含义 |
| --- | --- | --- |
| `ObservabilityAttributeKeys.BOT_ID` | `agentsflex.bot.id` | 宿主系统中的业务 Bot |
| `ObservabilityAttributeKeys.CONVERSATION_ID` | `gen_ai.conversation.id` | 一段连续会话 |
| `ObservabilityAttributeKeys.ACCOUNT_ID` | `enduser.id` | 发起交互的账号或最终用户 |
| `ObservabilityAttributeKeys.TURN_ID` | `agentsflex.turn.id` | 会话中的一次用户交互 |

Conversation 可以包含多个 Turn；一个 Turn 又可以包含多次 Chat、Tool 和 HTTP 调用。Turn ID 的价值是避免
把 Tool Calling 导致的多次模型请求误算成多次用户互动。

Turn ID 不由 Agents-Flex 自动生成，因为框架无法判断宿主业务中的一轮交互从哪里开始、在哪里结束。宿主
系统应在收到一次用户输入时生成一个 ID，并在处理这次输入产生的所有 Chat、Tool、HTTP 调用中复用：
