...
- **持久化 Agent Runtime**：提供 Snapshot 恢复、工具审批、版本 CAS、Middleware 和统一 AgentEvent。
...
| `agents-flex-agent` | 持久化 Agent Runtime：运行状态、Snapshot、版本 CAS、审批、Middleware 和事件 |
...
当前仓库版本见根 `pom.xml` 的 `revision` 属性，目前为 `2.2.9`。
