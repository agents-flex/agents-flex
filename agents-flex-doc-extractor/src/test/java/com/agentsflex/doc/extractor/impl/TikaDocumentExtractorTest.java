...
import com.agentsflex.doc.DocumentExtractionService;
...
public class TikaDocumentExtractorTest {
    ...
    public void shouldUseConfiguredHandlerForEmbeddedOpenDocumentImage() throws Exception {
        ...
        DocumentExtractionService service = new DocumentExtractionService((data, type, name) -> {
            imageBytes.set(data);
            mimeType.set(type);
            fileName.set(name);
        ...
        String text = service.extract(odt, "sample.odt",
    ...
    private String extract(byte[] data, String fileName, String mimeType) {
        return new DocumentExtractionService().extract(data, fileName, mimeType);
    }
