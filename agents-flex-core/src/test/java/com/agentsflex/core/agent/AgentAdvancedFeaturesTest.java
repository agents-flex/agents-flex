...
public class AgentAdvancedFeaturesTest {
    ...
    public void shouldClaimRunWithOnlyOneWorker() {
        ...
        store.releaseLease(run.getId(), "worker-a");
    ...
    public void shouldResolvePendingToolInAnotherRunner() {
        ...
        Agent agent = Agent.builder("remote-agent")
            .chatModel(model)
            .tool(tool("remote", args -> "configured"))
            .build();
        registry.register(agent);
        AgentToolRegistry externalRegistry = new AgentToolRegistry() {
            @Override
            public AgentToolReference register(String agentId, String agentVersion, Tool tool) {
                return AgentToolReference.builder(agentId, agentVersion, tool.getName())
            ...
            public Tool resolve(AgentToolReference reference) {
                return "remote".equals(reference.getToolName())
                    ? tool("remote", args -> executions.incrementAndGet()) : null;
            }
        };
        AgentRunner first = new AgentRunner(crashingStore, registry, externalRegistry);
        ...
        AgentRunner second = new AgentRunner(durableStore, registry, externalRegistry);
