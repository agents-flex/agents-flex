 ...
 * 恢复时由 {@link AgentRegistry} 根据 agentId 重新绑定这些对象。</p>
 *
...
public interface AgentRunStore {
    ...
    /** 延长指定 Worker 已持有的租约并返回新版本快照。 */
    default AgentRunSnapshot renewLease(String runId, String workerId, long leaseUntil) {
        throw new UnsupportedOperationException("lease renewal is not supported");
    }

    /** 释放指定 Worker 持有的租约。 */
    default void releaseLease(String runId, String workerId) {
    }
}
