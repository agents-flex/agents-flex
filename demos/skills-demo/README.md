...
Demo 只注册 `SkillsTool.buildTools()` 返回的 `skill`、`bash`、`read`、`write`、`edit`、`ls`、`glob` 和
...
文件大小。未配置会话 ID 时，本机默认输出到
`target/skills-demo-output/agentsflex-skills-runtime-report.pptx`。
配置 `SKILLS_CONVERSATION_ID` 后，Skill 会先复制到对应会话目录的 `skills` 子目录，产物和 Skill
运行时自行创建的文件都保留在该会话中，默认 PPTX 位于会话目录的 `output/` 子目录。
...
| `SKILLS_LOCAL_OUTPUT_FILE` | `target/skills-demo-output/<文件名>` | 仅远端模式使用的本机下载路径 |
